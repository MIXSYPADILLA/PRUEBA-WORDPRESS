...
if ( ! class_exists( 'Spark_Construction_Lite_' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Spark_Construction_Lite_ {
		 ...
		 * Single instance.
		 *
		 * @var Spark_Construction_Lite_ $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Spark_Construction_Lite_ ) ) {
				self::$instance = new Spark_Construction_Lite_;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s Info', 'spark-construction-lite' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s Info', 'spark-construction-lite' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-info';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'spark-construction-lite' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'spark-construction-lite' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'spark_construction_lite_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'spark-construction-lite' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_spark_construction_lite_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_spark_construction_lite_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap sparkconstructionlite-info-wrap">
			...
			</div><!-- .wrap .sparkconstructionlite-info-wrap -->
			<?php
		}
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'spark-construction-lite' ), $method );
			}
		}
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section sparkconstructionlite-section sparkconstructionlite-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .sparkconstructionlite-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section sparkconstructionlite-section sparkconstructionlite-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .sparkconstructionlite-section -->
		...
		public function useful_plugins() {
			...
			?>
			<div class="sparkconstructionlite-section sparkconstructionlite-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'spark-construction-lite' ); ?></a>
				<?php endif; ?>
			...
			</div><!-- .sparkconstructionlite-section -->
			<?php
		}
		...
		public function demo_content() {
			...
			?>
			<div class="sparkconstructionlite-section sparkconstructionlite-section-demo-content">
		...
		public function upgrade_to_pro() {
			...
			?>
			<div class="sparkconstructionlite-section sparkconstructionlite-section-upgrade-to-pro">
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'spark-construction-lite-info', get_template_directory_uri() . '/offshorethemes/admin/about-theme/css/info' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'spark-construction-lite-info', get_template_directory_uri() . '/offshorethemes/admin/about-theme/js/info' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'spark_construction_lite_dismiss_status', true );
			?>
			<?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
				<div class="sparkconstructionlite-info-notice notice notice-info">
					<?php $this->render_notice(); ?>
				</div><!-- .sparkconstructionlite-info-notice -->
			<?php endif; ?>
			<?php
		}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'spark_construction_lite_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'spark_construction_lite_dismiss_status', 1 );
