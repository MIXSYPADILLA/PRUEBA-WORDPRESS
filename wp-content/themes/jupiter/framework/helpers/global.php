...
if ( ! function_exists( 'mk_get_option_element_font_family' ) ) {
	/**
	 * Get theme option font family for selected element.
	 *
	 * @since 5.9.4
	 * @param string $selector HTML selector tag.
	 * @return string
	 */
	function mk_get_option_element_font_family( $selector ) {
		$font_family = '';
		$fonts = mk_get_option( 'fonts' );
		if ( $fonts ) {
			$font_family = $fonts[0]['fontFamily'];
			foreach ( $fonts as $font ) {
				if ( empty( $font['elements'] ) || ! is_array( $font['elements'] ) ) {
					continue;
				}
